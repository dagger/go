...
makes those settings ignored, recorded in `warnings`, rather than refused. Tool binaries come from `gomod`'s pinned
`tool-builder`, never built in the module's own container, and `gomod.withTool`
installs one only when the container does not already carry it.
...
`dagger.toml`. One suite, or one check, at a time:
